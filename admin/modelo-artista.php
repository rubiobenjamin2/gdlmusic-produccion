...
if($_POST['registro'] == 'nuevo') { //agregar-admin es el input de tipo hidden que se envia
   ...
   /* $respuesta = array(
    ...
    try {
        $stmt = $conn->prepare("INSERT INTO artistas (nombre_artista, url_img, url_pdf, editado) VALUES (?,?,?,NOW())");
        //$stmt = $conn->prepare(" INSERT INTO `invitados` ( `nombre_invitado`, `apellido_invitado`, `descripcion`, `url_imagen`, editado = NOW()) VALUES (?, ?, ?, ?) ");
        $stmt->bind_param("sss", $nombre, $imagen_url, $pdf);
        $stmt->execute();
    ...
    try {
        if ($_FILES['archivo_imagen']['size'] > 0) {
         ...
         //es recomendable usar esta tecnica cuando se use update
            ...
            $stmt = $conn->prepare('UPDATE artistas SET nombre_artista = ?, url_img = ?, url_pdf = ?, editado = NOW() WHERE id_artista = ?');
            $stmt->bind_param("sssi", $nombre, $imagen_url, $pdf, $id_registro);
            
        } else {
            //sin imagen
            $stmt = $conn->prepare('UPDATE artistas SET nombre_artista = ?, url_pdf = ?, editado = NOW() WHERE id_artista = ?');
            $stmt->bind_param("ssi", $nombre, $pdf, $id_registro);
